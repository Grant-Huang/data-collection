...
- 新增 `_find_cross_version_duplicates`：每条新记录与全部历史记录两两比较文本/结构相似度

**严重度分级——这是过程中真实发现并修正的一次校准，不是一开始就设计对的**：最初把文本相似度和结构相似度命中都设成阻断级错误，拿修复后的 40 条数据实测，结果 **40 条里 0 条能导入**——因为 652 处命中里绝大多数是"结构相似度高但文本完全不同"（不同行业、不同场景，只是共享同一套异常处置的 DAG 骨架），这正是实验设计文档 §4.4 明确说的"同一个 micro-workflow 故意在多个场景里重复出现"，是特意要保留的信号，不是该拒收的脏数据。如果把结构相似当成硬阻断，会导致以后几乎任何新场景只要用了这套骨架模板就永远导不进来。修正为：**文本相似度命中 → 阻断错误**（判定为"内容层面真的是同一条数据换了个 ID 重新传"，精确度高）；**结构相似度单独命中 → 仅警告，不阻断**（可能只是同模板的不同场景，交给 Prior 标注环节人工判断）。修正后再测：8 条精确重复（record_id 冲突 + 文本相似度 1.0）被正确挡下，其余 32 条真正的新内容全部放行，其余 644 处纯结构重叠以警告形式可见但不阻断。

**已验证**：真实场景下的完整闭环——同一批数据第一次上传部分导入（8 条已存在）→ 阻断这 8 条、放行 32 条新内容为 v3 → 再次上传完全相同的文件 → 40 条全部被 `record_id_already_exists` + `cross_version_near_duplicate_text` 挡下，确认不会被重复灌入。
