...
**仍未做（下一轮）**：
- 跨批结构命中要不要在 Prior 标注环节里显式呈现"这条和历史某条结构相似"供专家参考，目前只是预检报告里的一条警告文字，标注面板本身还没读取这个信号（**已在 §16.5 实现**：标注面板和列表现在会显示这些信号）
...
## 15. 采集会话引导重构：规划器 + 措辞器 + 分层气泡

**起因**（用户反馈）：会话话术生硬；追问跟不上流程需要；气泡设计需要改进。排查后的根因：LLM 只用于解析专家输入（`_understand_step`），专家看到的每一句回复都是写死在固定 Wizard 里的模板；主干只收两步就被带去问分支/并行/审批，长流程采不全；背景问题前置 7 轮；问题里还写死了与案例无关的例子；气泡没有复述层、没有自动滚动，选项与问题脱节，移动端不支持多选。

**决定（用户确认：三部分一起推进；回复生成接 LLM；背景问题不强制，服务于流程采集）：**

1. **规划器**（`guide_service.py`，确定性规则）：
   - 背景从 7 轮压到 2～3 轮：起因 → 目标（可跳过）→ 限制条件（多选，选多个时合并成一个追问，不再用"；"拼接多个问题）。开场直接讲经历的，跳过"发生了什么"。
   - 主干循环追问"「上一步」之后，下一步是谁做什么？"，直到专家说讲完（附"后面就处理完了"兜底选项）。
   - 先问结束条件、建 end 节点，再依次做结构**扫查**：分支 → 同时进行 → 等人确认 → 返工 → 经验（P6）。扫查选项都是专家自己说过的步骤名（PRD 18.2 "选项来自专家自己说过的话"=消歧，不是生成），没有可选步骤的扫查直接跳过。
   - 分支：把"原来那条路"作为情况 A（问条件），新增情况 B（条件 + 做法），再问"处理完接下来做哪一步"——选项是下游步骤，自由文字视为情况 B 的更多步骤。返工目标现在能定位到具体节点（`rework_reference_node_id` 不再恒为 null）。
   - 记住专家讲述中出现的"如果/签字/返工"等线索句，对应扫查开口时原样引用。
   - 修复否定句误判："不是同时做"不再被读成同时；扫查的"没有"类回答统一用 `_is_negative` 判断。
2. **措辞器**（新增 `guide_phrasing.py`）：规划器给出模板复述 + 模板问题，LLM（`guide_service` 槽位）改写得更口语化。输出必须通过校验，否则回退模板：引号内容和数字必须出自专家原话；不得出现"分支/并行/汇合/节点"等术语；只能有一个问题；保留问题里的锚点引用；**带选项的问题不改写问题本身**（防止改写后选项不再对应），只润色复述；例外是只带"后面就处理完了"这类通用兜底选项的主干追问，可以改写。已知局限：校验无法判断语义，模型若在复述里加入评价性的话（例如"这一步很关键"），校验拦不住，只能靠提示词约束。每轮最多 2 次 LLM 调用（解析 + 措辞）。
3. **解析器**：提示词新增 `actors`（专家原话里明确说出的执行人），不在原话中的一律丢弃，写入 `actor_roles`。
4. **回退**：扫查会用 `update_edge` 重连已有边，仅靠 `source_turn_ids` 删除后来的节点/边无法还原，因此 `_turn_state_log` 每轮额外保存 `graph_before` 快照，回退时整图恢复（旧记录无快照时仍走原删除逻辑）。
5. **旧会话迁移**：停在旧 Wizard 阶段（`branch_check`、`context_known` 等）的会话，下一轮自动接入新流程，不会卡死。
6. **气泡**（`MessageList`/`QuickReplies`/`index.css`）：AI 消息分三层——复述（灰色小字 + "图上 +N" 标签，桌面端悬停时高亮流程图对应节点）、问题、"为什么问"；选项挂在对应问题下方，兜底选项用虚线；已答问题的选项冻结保留并标出所选项；正在整理提示、自动滚动；桌面端与移动端共用一套组件，移动端支持多选；点选项时追加到已输入内容，不覆盖。`ConversationTurn` 新增可选字段 `ack/question/why/chips/chip_mode`，`text` 仍是完整文本，导出等下游不受影响。
7. `DagView` 布局刷新条件从"节点数/边数"改为内容签名，否则条件、审批人、重连边变化后图不更新。

**测试**：新增 `backend/tests/`（`pytest`，每个测试使用独立的临时 SQLite），覆盖完整对话到可提交、歧义连接词与否定句、同时进行的重连、背景精简、快照回退、旧阶段迁移、措辞校验与回退。

**关于 7B 是否够用**：这套分工下，7B 只做两件受约束的事——按固定 JSON 结构解析，以及在校验下润色措辞；"问什么"由规则决定。Qwen2.5-7B-Instruct 官方模型卡称其在指令遵循和"generating structured outputs especially JSON"上有明显提升（https://huggingface.co/Qwen/Qwen2.5-7B-Instruct），与这两项任务匹配。实际质量仍需在真实 endpoint 上用若干真实对话验收，重点观察校验回退率。

**补充：「刷新工作流图」后重置对话进度。** 刷新会整体替换流程图，旧的对话状态（光标、扫查选项、分支/更正的中间状态）指向的节点都已不存在，继续对话会出错。现在刷新成功后由 `guide_service.state_after_regeneration` 重置：保留背景信息、线索记忆和已答过的扫查；刷新时正处在某一类扫查中途的，那一类在新图上重新问；随后从新图的结构扫查继续（选项按新图的步骤重建），全部问过则直接进入最终确认。同时清空逐轮回退快照（快照里是刷新前的旧图，跨刷新回退会把旧图换回来），并追加一条助手消息说明图已重新整理。确认状态随之调整：还有待答问题时为“采集中”，否则为“待确认”。前端刷新前的确认弹窗也写明了这些影响。

## 16. 流程标注易用性改造：盲标、队列、结构化原因、返工闭环

> **注**：本节的"返工"步骤、返工编辑器和"点节点判定"面板已被第 17 节的统一审阅循环取代（标注人在对话里直接改图）；盲标、结构化原因、标注队列、机器信号等仍然有效。旧的返工数据按 17.4 的兼容规则继续读取。

对 Prior / Gold 标注（§9.2、§9 Phase C-2）做了一次核查，发现两个影响数据质量的缺陷和一批易用性问题，参考业界标注工具的通行做法（Prodigy 的"一次一条 + 快捷键"、Label Studio / Argilla 的标注队列与重叠标注、Prodigy review recipe 的分歧并排对比、Labelbox 的 Rework 队列）整体改造。用户确认的三个决策：

- **决策 8**：双人独立标注**全量**做，不抽样
- **决策 9**："需要修改"之后必须**真正产出修正后的图**，并走返工（Rework）流程
- **决策 10**：丢弃 / 修改原因改成**结构化标签**，方便统计

### 16.1 修掉的两个数据质量缺陷

1. **"独立标注"其实不独立**：Phase C-2 虽然不再预填上一次判定，但面板底部仍显示完整标注历史（"采纳（张三）→ …"），Dashboard 列表标签也写着"已标注・采纳"——第二位标注人能直接看到第一位的结论，Gold 和 κ 都会被高估。现在改为**后端盲标**：记录处于 `first_review` / `second_review` 阶段时，详情接口不返回任何人的判定、备注、节点判定和返工记录，只返回本轮已标注人姓名（用来提前拦同一个人重复标注）和返工人姓名；列表只返回**已定结论**（`final_verdict`，阶段为 `done` 时才有）；汇总里的判定分布 / 原因分布也只统计已结束的轮次，避免小版本里从聚合数反推。机器预检信号不是人的结论，盲标阶段照常显示。
2. **"合并进上一个节点"用的是列表顺序**：原实现取 `nodes[i-1]`，图有分支 / 并行时那个节点根本不和当前节点相连。现在合并目标只能是**图上真实的前驱节点**（前端只提供这些选项，后端 `rework.validate_edits` 再校验一次）。

### 16.2 状态模型：轮次 + 阶段（`app/gold_annotation.py`）

- 每条记录按**轮次**推进：第 1 轮审原始图；每次返工提交（`record_revisions` 表一行）产出修正图，开启下一轮。当前图 = 最新修订的图，没有修订就是原图；已发布的 `dataset_versions` 行仍然不可变。
- 轮内规则沿用 C-2：两次独立标注一致即定结论，不一致由第三人仲裁。新的是**结论的含义**：采纳 → `done` + Gold；丢弃 → `done` + 非 Gold；需要修改 → `rework`（原来是死胡同："非 Gold"，建议的修改从未落地）。
- 阶段 `first_review / second_review / arbitration / rework / done` 驱动标注队列；`GoldStatus` 新增 `needs_rework`。
- 独立性校验按**本轮**计算，另加一条：返工人不能在下一轮复核 / 仲裁自己的返工（同"不能自我确认"的原则）。
- 提交标注 / 返工时带上客户端看到的 `round`，记录期间已进入新一轮则返回 409，不会把判定记到标注人没看过的图上。
- κ：每一个有两条独立标注的轮次都算一对，返工后的轮次也是真实的双人观测；`annotation_readiness` 的双人覆盖率按"任一轮有两条独立标注"计。

### 16.3 返工（`app/rework.py` + `POST .../rework/preview`、`POST .../rework`）

- 编辑集合 `{node_verdicts, renames, inserts}`：删除（前驱直连后继，保留入边的类型 / 条件，决策分支的条件不会丢）、合并进真实前驱（标签拼成"A；B"，出边转给前驱）、改名、在某节点后插入步骤（新节点接管原出边；分支 / 并行拆分节点后不允许插入，提示去具体分支上插）。标注人的 `node_verdicts` 本身就是合法的编辑集合，所以返工人可以直接"以某位标注人的建议为起点"。
- 应用顺序固定为**合并 → 删除 → 插入 → 改名**：先在标注人看到的原结构上做结构判断，再插入，最后改名（这样改名可以覆盖合并后的"A；B"标签）。实现过程中发现先插入再合并会让新节点成为被合并节点的前驱、重新接线后成环，这是调整顺序的直接原因。
- 预览和提交走同一段 `apply_edits` + `graph_validator`：预览返回修正后的图和校验问题，提交时有 error 级问题直接拒绝。
- 标注人提交"需要修改"时，节点判定也用 `validate_edits` 校验，保证返工人拿到的建议一定能应用。

### 16.4 结构化原因（决策 10）

`reason_tags`：步骤缺失 / 多余步骤 / 顺序错误 / 重复 / 分支条件错误 / 角色错误 / 描述不清 / 不属于该场景 / 其他。选"需要修改"或"丢弃"时至少选一个，选"其他"时备注必填；"采纳"时清空。汇总接口新增 `reason_tag_counts`，Dashboard 显示出现最多的几类问题。

### 16.5 机器信号进标注面板（`app/annotation_signals.py`，关闭 §10 留下的待办）

标注面板和列表显示三类规则信号：当前图的 `graph_validator` 问题 + "全部节点无角色"；版本内近重复 / 疑似微工作流复用（复用 `import_pipeline.compare_within_batch`，与导入时同一套分类，按版本 + 阈值缓存，因为已发布版本的记录不会变）；导入时的跨版本命中（`import_confirm` 现在把 `version_*` 类问题存进 `version["precheck_issues"]`）。**如实说明**：本次改动之前导入的版本没有存跨版本命中，只显示前两类——事后再和"该版本之前发布的所有数据"比，比对集合和导入当时不同，所以不补算。列表可按"机器信号多的优先"排序。

### 16.6 前端易用性

- `AnnotationQueue.tsx`（从 Dashboard 抽出）：按阶段筛选，默认"我可处理"（根据本轮参与人姓名提前判断，不用做完才被后端拒绝）；进度条；"从第一条开始连续处理"。
- `PriorAnnotationPanel.tsx`：标注人姓名记在浏览器里（`useAnnotatorName`，与 `useRole` 同一模式）；快捷键 1/2/3 选判定、Enter 提交并下一条、←/→ 切换记录、Esc 关闭（正在输入时不触发）；打开时把当前筛选结果冻结成队列，状态变化时导航顺序也不乱；点图上的节点即可逐个判定（自动切到"需要修改"），删除 / 合并在图上用颜色标出；仲裁时两份独立标注并排显示，图上用橙色标出两人判定不同的节点；返工时左边当前图、右边服务端实时预览；已完成的记录显示按轮次分组的完整历史，可切换原始图 / 最终图。
- 图改成"按宽度适配 + 框内纵向滚动"（移动端 DAG 页已用的模式），原来塞进 300px 高的框里整体缩小，看不清字。
- `DagView` 布局 key 从"节点数-边数"改为节点 id + 边端点：返工预览会出现"合并一个、插入一个"这种数量不变但结构变了的情况。
- Dashboard 的"Gold 数量""待复核"两张卡片原来写死 0（提示语还写着"标注体系尚未实现"），改为读取真实统计。

### 16.7 已验证

1. 后端端到端（FastAPI TestClient + 临时 SQLite，走真实 HTTP 接口）16 项检查全部通过：盲标时详情 / 列表都不泄露判定；同一人不能做第二次独立标注；"需要修改"/"丢弃"必须带原因，"其他"必须带备注；合并目标必须是真实前驱；开始节点不能删除；一致"需要修改"→ 返工阶段且此时拒绝标注；产生非法图的返工能预览出问题、提交被拒；分支节点后不能插入；合并 + 改名 + 插入接线正确；返工后进入第 2 轮盲标；返工人不能复核自己；过期轮次返回 409；第 2 轮一致采纳 → Gold；分歧 → 仲裁 → 丢弃；汇总计数正确（κ=0.556，手算核对：4 对，po=0.75，pe=7/16）。
2. 浏览器实测（Playwright + Chromium，真实后端 + 构建后的前端）：alice / carol 的"我可处理"数量符合预期；盲标页面看不到第一位的结论；按 1 + Enter 提交后自动跳到下一条；仲裁时点节点选"合并进前驱"、选原因、Enter 提交 → 进入返工；返工"以此为起点" + 插入步骤 → 预览正确 → 提交 → 第 2 轮；"丢弃"未选原因时提交按钮提示并禁用；刷新后姓名仍在；无页面报错。
3. `tsc -b` 和 `vite build` 通过。

### 16.8 仍未做（如实记录）

- 身份仍是自由填写的姓名（假设 1 的老问题），挡得住无意识重复，挡不住存心冒名。
- 返工编辑只支持删除 / 合并 / 改名 / 插入单个活动节点；改边的类型或条件、增加分支、改角色，需要一个完整的图编辑器，本轮没做。
- 原因标签是固定列表，没有按行业自定义。

## 17. 统一审阅循环：讲述 → 草图 → 对话澄清与修改 → 满意即完成（新建 / 编辑 / 标注共用）

**起因**（用户反馈）：逐步引导的问答方式太生硬，气泡选项也属于"提示"。希望专家一次（最多一两次）把完整流程讲清楚，可以打字也可以语音，之后由 Agent 只做事实澄清。进一步提出：专家讲完第一遍之后，新建流程和标注其实是同一件事——多轮修改直到满意；满意之后，编辑就是编辑完成，标注就是标注完成。

**用户确认的决策：**

1. 新建、编辑、标注统一成一个**审阅循环**，只有起点和"满意之后保存成什么"不同。
2. **取消第 16 节的单独返工步骤**：标注人在对话里直接改出修正后的图；两人修正一致则直接采用，不一致由仲裁人选一份或在其中一份上继续改。
3. 整段讲述生成流程图、以及审阅对话都用 **C_standard** 级别模型。`guide_service` 槽位默认级别由 L 改为 C_standard；**只改默认值**，已保存过设置的部署需在"系统管理"里手动调整（`settings.get_effective_settings` 以数据库中保存的设置为准）。
4. Agent 主动澄清的问题数**不做严格限制**：默认上限 20 个（可在设置 `review.max_clarify_questions` 调整）；问完或到上限后进入复述确认，人随时可以继续提修改，直到说满意为止。
5. 语音**先只做语音转文字**（接入 Qwen3-ASR-Flash-Realtime，桌面端和移动端），实时语音对话以后再做。识别只输出一份原文；Agent 的"理解"作为它自己的回复单独显示，不做识别后润色。

### 17.1 三种场景的对应关系

| | 起点 | 对话中 | "满意"之后保存为 |
|---|---|---|---|
| 新建 | 专家的整段讲述 → Agent 生成草图 | 澄清 + 修改 | 该流程的待确认版本，再次确认即提交（`expert_confirmed`） |
| 编辑 | 已确认、尚未进入数据集的流程（"继续修改"） | 修改 | 同上 |
| 标注 | 待审的流程图（每位标注人各自一份副本，互不可见） | 修改 | 一条标注记录：结论 + 原因标签 + 改动 + 修正后的图 |

专家自己确认只算作者确认；Gold 仍需两位其他人独立标注。

### 17.2 新建流程

1. 开场：Agent 给出一段示范讲述，说明怎样讲比较好（起因、按顺序谁做什么、遇到什么情况走不同处理、哪些同时做、要等谁签字、什么情况返工、怎样算做完、哪里靠经验），不给选项。
2. 专家讲述（打字或语音，语音转写后可先修改再发送）。第一条内容太短（不足约 60 字）时，Agent 请专家把整个过程讲完整，不调用模型。
3. 生成草图（`graph_regenerate` 槽位，C_standard）：模型输出节点、边、每个节点的**原话依据**、一段复述、不确定点清单、背景信息（起因/目标/限制）。
   - 原话依据由程序核对是否真的出现在专家的话里。**核对不通过的节点不删除**（删掉会让图断开），而是标记为"未找到依据"，列为最先要问的澄清问题——这一点比讨论时说的"直接丢弃"更稳妥。
4. 进入审阅循环。之后专家再说的任何话（补充、纠正、回答）都由审阅 Agent 处理，不区分"还在讲述"还是"在回答问题"。

### 17.3 审阅循环（`app/review_agent.py`）

每一轮一次模型调用（`guide_service` 槽位），输入：当前图（带节点编号）、对话记录、待澄清清单中排在前面的几项、这一轮的人的发言。输出（JSON）：

- `intent`：修改 / 回答 / 满意 / 丢弃（仅标注）/ 撤销 / 其他；
- `understanding`：一句话复述"我理解你的意思是……"；
- `ops`：图修改操作，沿用 `graph_ops` 的变更协议（PRD 第 16 节"LLM 修改与人工修改共用一套变更协议"）；新增节点须带原话依据；
- `resolved_gap_ids`：这一轮解决了哪些待澄清项；
- `next_question`：从给定的待澄清项里挑一项，用自然语言问出来。

程序侧把关：操作先在副本上执行并过 `graph_validator`，有 error 就整轮不改图，并如实告诉对方"这次的修改没能落到图上"；问题必须对应给定的待澄清项，否则换成模板问题。每一轮对图的改动以文字列出（"新增步骤「通知班组长」""删除步骤「复测」"……），和"图上 +N"一起显示在 Agent 气泡里。

**待澄清清单**（规则生成 + 模型补充，`app/review_gaps.py`）：未找到原话依据的节点 → 判断节点缺条件 → 结构校验错误 → 模型标出的不确定点 → 一次性的覆盖检查（有没有例外情况 / 同时做的事 / 要等谁确认 / 返工 / 靠经验的判断，各问一次）→ 没有执行人的步骤（合并成一问）。

**结束**：人表达满意 → Agent 用一段话复述整个流程并请对方确认 → 对方确认即完成；对方提出修改则回到循环。撤销：说"撤销刚才的修改"可恢复到上一轮的图（沿用每轮 `graph_before` 快照）。

**模型不可用时**：新建流程如果 C_standard 未配置，仍走旧的逐步引导（`guide_service`，保留为兜底，默认不出现）；对话中途调用失败，本轮不改图，如实提示稍后再试。标注在模型不可用时只支持规则识别的"没问题 / 丢弃 / 撤销"，不能按描述改图，会如实说明。

### 17.4 标注（取代第 16 节的面板和返工步骤）

- 每位标注人打开记录时得到自己的**审阅会话**（`annotation_sessions` 表），起点是该记录当前的图；Agent 只看得到这张图和这位标注人自己的话，看不到其他人的结论（盲标不变）。
- 标注人表示满意时，Agent 给出结论建议并请确认：没改过图 → 采纳；改过图 → 需要修改，原因标签由模型根据对话建议（不在词表里的丢弃），模型不可用时按改动类型推断（新增步骤 → 步骤缺失，删除 → 多余步骤，合并 → 重复，改连线 → 顺序错误，改条件 → 分支/条件错误，改执行人 → 角色错误，改名 → 描述不清）；说"这条不能用" → 丢弃。确认后生成标注记录，保存结论、原因标签、改动操作、修正后的图。
- **结果判定**（`gold_annotation.compute_state`）：
  - 两人都采纳 → Gold（原图）；两人都丢弃 → 结束，非 Gold；
  - 两人都"需要修改"且修正后的图结构一致（按步骤名称、连线、条件比较）→ Gold（修正图）；
  - 其余情况 → 仲裁。仲裁人看得到两份结论和修正图，可以说"用张三的版本"、在某一份上继续改、采纳原图或丢弃；仲裁结论即最终结果。
- 第 16 节已有数据的兼容：旧的"返工修订"仍按原规则开启新一轮；旧标注没有修正图，两人都判"需要修改"时进入仲裁，由仲裁人在对话里改出修正图。
- 一致性系数 κ、原因统计、盲标、标注队列和"我可处理"筛选保持不变；删除第 16 节的"待返工"阶段、返工编辑器和"点节点判定"面板（改为：点图上的节点会把节点名填进输入框，方便引用）。

### 17.5 语音转文字

- 后端新增 WebSocket 代理 `/api/voice/asr`，使用"系统管理 → 语音"里已有的 `voice.api_key / workspace_id / realtime_model` 配置连接 Qwen3-ASR-Flash-Realtime，自动断句（VAD）模式，16kHz PCM。连接方式与 `web-demo/server.py` 的代理一致。
- 前端桌面输入框和移动端都加麦克风按钮：边说边显示识别结果，停止后文字进入输入框，专家可修改后发送。未配置语音服务时，退回浏览器自带识别（国内网络下可能不可用）。
- 保存：专家消息的 `text` 是确认后的原文；使用语音时另存 `raw_transcript`（识别原始结果，不可修改），便于核对识别错误。

### 17.6 实现与验证记录

**代码**：`review_agent.py`（审阅循环引擎）、`review_gaps.py`（待澄清清单）、`routers/voice.py`（语音代理）、`routers/expert_workflows.py`（审阅模式 + `POST /{id}/reopen`「继续修改」）、`routers/annotations.py`（`POST .../review-session` 开始/继续标注会话，`POST .../review-sessions/{id}/turns` 对话；删除返工接口和 `rework.py`）、`gold_annotation.py`（去掉"待返工"阶段；`round_decision` + `graph_signature` 判定）。前端：`useVoiceDictation` + 桌面 `VoiceDictationButton` + 移动端 `VoiceCapsuleInput`；`MessageList` 显示"理解 / 改动列表 / 复述 / 讲述示范 / 语音原文"；`PriorAnnotationPanel` 改为左图右对话；删除 `ReworkEditor`、`NodeActionBar`。

**已验证**：
1. 后端 `pytest` 24 项全部通过：新增审阅循环（讲述太短不调用模型、生成草图并核对原话、未找到原话的步骤先问、改图并列出改动、会引入环的修改整轮拒绝、撤销、回答"对"不被误判为满意、复述确认、模型出错不改图、澄清上限、继续修改）、标注对话（两人采纳 → Gold、两人同样修改 → Gold（修正图）、分歧 → 仲裁采用某人版本、"这条不能用" → 丢弃、仲裁期间另一会话失效、旧的仅结论标注进入仲裁、无模型时只支持短句意图）、语音代理（模拟上游：鉴权头、固定会话配置、事件白名单、识别结果回传）。原有 13 项（逐步引导兜底等）不受影响。
2. 浏览器实测（Playwright + Chromium 虚拟麦克风，真实后端 + 构建后的前端 + 模拟的 OpenAI 兼容模型和模拟 Qwen 语音上游）：语音讲述 → 识别文字进入输入框 → 发送 → 生成草图（未找到原话的步骤虚线标出并最先提问）→ 对话修改（列出"新增 / 删除"）→"没问题了" → 复述 →"确认"提交 →"继续修改"按钮出现；标注：alice 在对话中改执行人并确认、bob 直接确认 → 分歧进入仲裁 → carol 说"用 alice 的版本" →"确认" → Gold（修正图含 alice 的修改）；手机尺寸下语音口述回填正常。无页面报错。
3. 实测中发现并修复：导入记录的节点没有 `source_turn_ids`，消息列表统计"图上 +N"时崩溃（白屏）；完成度量纲不一致（显示成 4710%）；标注提交后错误显示"你已标注过本轮"提示；标注面板缺字体设置；手机上录音条溢出屏幕、停止按钮点不到（旧组件同样存在）；改名被误判为"改连线"导致多出"顺序错误"标签；模型给出的原因标签未传到最终结论。

### 17.7 已知限制（如实记录）

- 本沙箱没有可用的 Qwen 语音服务和大模型服务：语音代理用模拟的上游服务验证协议转发，审阅对话用模拟的 OpenAI 兼容服务验证流程；真实识别效果和模型输出质量需要在部署环境验收。
- 模型输出质量（抽取是否漏步骤、改图是否准确、提问是否自然）只能在真实的 C_standard 模型上验收；代码侧保证的是：改图前校验、原话核对、复述由图生成、失败不改图。
- 移动端输入框仍是单行，长段口述回填后不便逐字修改；移动端也还没有"继续修改"入口。
- 讲述阶段第一条内容足够长就立即整理；专家若想分两段讲，第二段会作为补充交给审阅 Agent 处理（能加步骤，但不会重新整体生成；需要时可用"刷新工作流图"）。

### 17.8 数据集导出：原始图和 Gold 图都导出（用户决定"两个都要"）

`GET /api/datasets/versions/{id}/export` 的每条记录（三种格式都一样）：
- `graph`：记录原本的流程图（采集 / 导入时的样子），不变，导出文件仍可原样再导入；
- `gold_graph`：Gold 流程确定的图——标注人或仲裁人采用了修正时是修正后的图，按原样采纳时是记录当前的图；记录还不是 Gold（标注中或已丢弃）时为 `null`。角色归一化 / 匿名格式对两张图做同样的处理；
- `annotation`（schema v2 `workflow_record` 里本来就有的对象，导入记录已有的字段保留）补充：`gold_status`、`final_verdict`、`corrected`、`reason_tags`、`annotator_count`、`adjudicated`。结论和原因只在记录有最终结论后才写入，与标注列表的盲标规则一致。
...
- **两张图、两个 Tab、互不联动**：上层「任务协作」（谁负责哪一段、任务间怎么交接）、下层「SOP 步骤」（每一段具体怎么做）。桌面右栏和移动端 DAG 页都用 Tab 切换；点击任务**不会**跳转或过滤 SOP Tab（明确要求"两个 Tab 各自独立"）。
- **上层 DAG 的来源优先级**：① 专家直接定义（最可靠，适合首次建立流程）> ② 大模型从真实工作中自动提取（长期最重要，适合持续发现和优化）> ③ 系统根据负责人/组织结构推断（只能作辅助，不能单独规定 DAG）。原则：负责人决定"谁负责"，专家/真实工作决定"任务怎么协作"，大模型负责把它结构化。**本轮只实现 ①**，② 是双 DAG 设计文档 §26 P2 的执行日志挖掘，③ 本轮完全不做（刻意不按 `actor_roles` 自动分组：步骤图里只有审批节点带角色，且"谁负责"不等于"任务在哪里切开"）。
- **复用是数据挖掘的结果，不是专家定义的**，本轮不实现复用挖掘，只在数据模型里留好"任务 → 它包含哪些 SOP 节点"的映射，作为以后挖掘的输入。
- **数据集/导出也带上上层**。

### 18.2 实现

- **数据模型**（`models.py`）：`WorkflowRecord.graph` 保持原样，语义上就是 SOP 步骤层（Dataset/Dashboard/实验中心所有既有消费方零改动）；新增可选 `task_workflow: {graph: Graph, tasks: [TaskDefinition]}`。上层本身也是一张普通 `Graph`（开始 → 任务节点 → 结束），所以 `DagView`、schema 校验、导出匿名化全部直接复用。任务名在节点 `label`、负责方在节点 `actor_roles`，**不在 `tasks` 里重复存**——这样导出匿名化（本来就处理节点 label/角色）自动覆盖上层，不需要第二套规则。`TaskDefinition` = `task_id` + `sop_node_ids` + `definition_source`（`expert_defined` / `llm_mined` / `org_inferred`，对应上面的三级来源，本轮只会产出第一种）+ `structured_by`（`llm` / `rule`）。
- **会话流程**（`guide_service.py` + 新增 `task_layer.py`）：接在第 15 节规划器的唯一收尾出口 `_to_review` 上——所有结构扫描（分支/并行/审批/返工/经验）都跑完、原本要进 `review` 时，先插两个阶段：
  1. `task_outline`：问"按「谁负责哪一段」分成几个任务？按先后说，每项写成「负责方：做什么」"。专家的回答由 `guide_service` 环节的 LLM **只做结构化**（拆项、分出负责方），LLM 返回的负责方如果在原话里找不到，丢回"未说明"（防编造）；LLM 未配置/失败时退回规则解析（闭集分隔符 `→ ； 换行`，没有强分隔符时才用 `， 、 然后`；每项识别 `X：Y` / `X负责Y`，识别不出就不填负责方，不猜）。另有「整件事就一个任务」气泡，任务名取专家开场自己选的类别。
  2. `task_boundary`：从第二个任务起，逐个问"「X」是从哪一步开始的？"，气泡 = 已描述过的步骤（按讲述顺序，只列上一个边界之后的），外加「（这个任务没有对应的步骤）」——专家在上层提到、但没有逐步讲过的任务很常见，硬挂到某一步就是编造映射。不点气泡直接打字会被礼貌地请回选项（跟更正流程的选轮次一样，不做模糊匹配）。
  - 步骤顺序用 SOP 图的**拓扑序**（同层按创建顺序），不是 `nodes` 列表顺序——第 15 节的扫描会把判断/审批节点"插到"已有步骤后面，但它们追加在列表末尾，按列表切会把插入的审批节点错分到最后一个任务。边界存节点 id。每个 SOP 节点归属到"开始位置 ≤ 它自己位置"的最后一个任务；相邻任务负责方都说了且不同，中间的边是 `handoff`，否则 `normal`。
  - `task_workflow` 存在 FSM 的 `pending` 里，路由在每个同步 `case_context` 的地方一起同步它——所以更正回退（恢复到某一轮之前的状态）会自动连上层一起回退。「刷新工作流图」（`state_after_regeneration`）不保留 `task_workflow`（它的 `sop_node_ids` 指向被丢弃的旧图），扫描跑完后会重新问一遍任务划分。确认提交时上层节点/边一起标记 `expert_confirmed`。
- **前端**：新增 `DualDagPanel`（两个 Tab，默认 SOP Tab——它是对话中实时长出来的那张；上层要到对话最后才出现，之前显示对应的等待说明；上线前采集的老会话显示"没有任务协作图"）。任务节点下多一行"负责：质量部 · 3 个步骤"。移动端 DAG 页同样两 Tab，SOP Tab 下保留结构统计/规则与经验，任务 Tab 下换成"任务与负责方"列表，待确认信息两边都显示。
- **导出**：`records_for_export` 带上 `task_workflow`；`role_normalized` / `anonymized` 导出对上层图应用跟步骤图完全相同的角色归一化/人名脱敏。`schema/workflow_graph_schema_v2.json` 的 `workflow_record` 新增可选 `task_workflow` 属性（`graph` 复用同一个定义），所以导出文件仍能被导入流水线的 schema 校验接受。

### 18.3 已知限制（诚实记录）

- **只覆盖逐步引导（第 15 节）这条采集路径。** 第 17 节的「先讲述、后评审」循环（`review_agent`，配置了 C 档模型时新建会话默认走它）目前不产出任务层，任务 Tab 显示说明文字。要覆盖它，需要在评审循环里加一个"按负责方划分任务"的环节——改动的是第 17 节的对话设计，需要产品确认后再做。同理，已确认的流程通过「继续修改」进入评审循环后改了步骤图，旧的 `sop_node_ids` 可能指向已删除的节点（前端只统计仍存在的节点，不会报错，但映射需要重新划分）。

- 上层目前只能是**顺序链**。"涉及质量才交给质量部"这类跨负责方的条件分支，规则解析做不到，LLM 这一版也没让它输出分支——等 ② 执行日志挖掘或下一轮再加。
- 任务边界按**讲述顺序**切。步骤图有分支时（B 分支的节点创建在 A 分支之后），按顺序切可能把两条分支切到不同任务，需要专家在图上核对。
- 专家目前不能在图上直接改上层（跟 SOP 层一样，改图要回到对话里说）。

### 18.4 复用挖掘（下一轮）——挖出来的东西可以定义成"简单 Skill"

可以，而且建议就这么定义，因为这一轮留下的 `task_workflow.tasks[].sop_node_ids` 正好就是挖掘的输入单元（"一个任务内部的一段 SOP 子图"）：

```
Skill（挖掘产物，不是专家定义）
  skill_id, version
  name                    # 取支持样本里出现最多的任务名，专家可改
  sop_template            # 归一化后的子图（节点类型 + 规范化标签 + 边），不含具体人名/批次
  interface               # 入口/出口（进入这段之前、离开之后分别接什么）
  support: [{workflow_id, task_id}]   # 哪些会话里的哪个任务命中了它（出处，可追溯）
  support_count, domains  # 出现次数、跨了哪些业务域（生产/客诉/售后…）——跨域才是第二层复用的价值
  status: candidate | validated       # 挖出来只是候选，要人确认才算可复用
```

挖掘可以直接从现有能力起步：`import_pipeline.py` 的近重复检测已经能区分"场景不同、结构相似"（`microflow_reuse_candidate`），把比较单位从"整条记录"缩小到"一个任务的 SOP 子图"，就是 Skill 候选。另外建议：被多个工作流引用的 Skill 默认**锁定版本号**，升级显式操作——否则对 Strict SOP（合规/高风险）来说，"改一次处处生效"等于悄悄改了已审核过的流程。
