...
## 10. 已知缺口：近重复检测只查批内，不查跨版本

在实际导入两批公共集数据时发现的真实产品缺口，不是本轮 Phase 7 设计里预见到的，单独记一节。

**现状**：`import_pipeline.precheck` 的近重复检测（12.2.2）只在**当前这一次上传的 payload 内部**两两比较（`_find_near_duplicates` 只吃 `records` 这一个列表），从来不会去和数据库里**已经导入过的历史 `dataset_version`** 比。

**这在实际使用中是真问题，不是理论风险**：导入第一批 20 条公共集（`manufacturing_workflows_consolidated_v2.json`）生成 v1 之后，再导入第二批 40 条（`workflow_graph_v2______40_.json`）生成 v2，系统的预检报告对这两批之间的重复完全没有反应——因为它们是分两次上传的，各自的批内查重都不会看到对方。手工用 `import_pipeline._jaccard`/`_record_structure_sets` 把 v1 的 20 条和 v2 上传时的 40 条原始记录做了一次跨批比较，找到 **78 组结构近重复**（结构相似度 0.75-1.0，文本相似度普遍很低——两批数据在具体场景描述上不同，但共享同一套"发现异常→暂停/隔离→复测确认→…→审批→结束"DAG 骨架模板），而这些在当时的预检报告里一条都没有出现。

**对产品的实际影响**：随着公共集分批导入的次数增多，跨批的结构同质化会被系统性地漏检——Dataset Readiness Score 的 `low_leakage_risk` 维度和预检报告里的"近重复"提示都只反映单次上传批次的情况，看起来干净，但把多个版本的数据合并起来看（比如导出全部版本喂给下游实验）实际重复率可能远高于任何一次预检报告显示的数字。

**下一轮实现方向（草案，本轮不做）**：
- `precheck`/`import_confirm` 增加一个可选步骤：把本次上传的 records 和**同一 `source_type` 下所有已发布版本**的 records 做跨批比较，而不只是批内比较；复用现成的 `_jaccard`/`_word_shingles`/`_record_structure_sets`，不需要新算法
- 跨批命中的处理方式需要产品决策：是像批内一样只报 warning（不阻断导入），还是要求专家在 Prior 标注环节里显式确认"知道这条和历史某条相似，仍然要收录"——这类似标注环节要不要吸收查重结果的问题，留给下一轮和标注流程一起设计
- 计算量会随着历史数据量增长而变大（每次导入都要和全部历史记录比较一遍），届时可能需要索引/分桶优化（比如先按 `workflow_type`/`scenario_category` 分组再比较），不需要现在就做，等数据量真的大到有性能问题时再优化
