...
**仍未做（下一轮）**：
- 跨批结构命中要不要在 Prior 标注环节里显式呈现"这条和历史某条结构相似"供专家参考，目前只是预检报告里的一条警告文字，标注面板本身还没读取这个信号（**已在 §15.5 实现**：标注面板和列表现在会显示这些信号）
...
### 15.2 状态模型：轮次 + 阶段（`app/gold_annotation.py`）
...
### 15.3 返工（`app/rework.py` + `POST .../rework/preview`、`POST .../rework`）
...
### 15.5 机器信号进标注面板（`app/annotation_signals.py`，关闭 §10 留下的待办）
