...
## 15. 双层工作流：任务协作 DAG + SOP 步骤 DAG（一次专家输入同时产出两张图）
...
- **会话流程**（`guide_service.py` + 新增 `task_layer.py`）：原来 `end_condition` 之后直接进 `review`，现在中间插两个阶段：
  ...
  - 每个 SOP 节点归属到"开始位置 ≤ 它自己位置"的最后一个任务；相邻任务负责方都说了且不同，中间的边是 `handoff`，否则 `normal`。
  - `task_workflow` 存在 FSM 的 `pending` 里，路由用 `_sync_state` 同步到记录上——所以更正回退（恢复到某一轮之前的状态）会自动连上层一起回退。确认提交时上层节点/边一起标记 `expert_confirmed`。
...
### 15.4 复用挖掘（下一轮）——挖出来的东西可以定义成"简单 Skill"
