...
from .. import db, explain, quality
...
def publish_dataset(req: PublishDatasetRequest) -> DatasetVersionSummary:
    ...
    readiness = quality.compute_readiness(graphs)
    ...
    version = {
        ...
        "name": req.name or f"{req.source_type} v{version_number}",
...
def list_versions(source_type: str = "expert_collected") -> list[DatasetVersionSummary]:
    return [_to_summary(v) for v in db.list_dataset_versions(source_type)]
