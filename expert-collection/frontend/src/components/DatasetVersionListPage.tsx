...
//
// 公有集下的每个版本都是独立导入进来的一份数据集（不像专家集是持续发布的单一版本流），所以
// 只有公有集在这里提供改名/删除；删除是真删除（不是归档那种可撤销的隐藏），需要 canManage
// （管理员）权限，跟本页其他管理动作（归档/Gold 标记）用的是同一条权限约定。
...
export function DatasetVersionListPage({ sourceType, canManage, actorRole, onBack, onSelectVersion, onChanged }: Props) {
  ...
  const canRenameDelete = canManage && sourceType === "public_extracted";
  ...
  async function handleRename(versionId: string) {
    ...
    setError(null);
    try {
      await api.renameDatasetVersion(versionId, name, actorRole);
      setEditingId(null);
      await load();
      onChanged();
    } catch (e) {
  ...
  async function handleDelete(version: DatasetVersionSummary) {
    if (!window.confirm(`确定要永久删除数据集「${version.name}」吗？此操作不可恢复。`)) return;
    setError(null);
    try {
      await api.deleteDatasetVersion(version.id, actorRole);
      await load();
      onChanged();
    } catch (e) {
      setError(String(e));
              ...
              <tr key={v.id} style={{ borderBottom: "1px solid #f1f3f5" }}>
                  ...
                  {editingId === v.id ? (
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <input
                        ...
                        style={{ border: "1px solid #d0d5dd", borderRadius: 6, padding: "3px 8px", fontSize: 12.5, width: 140 }}
                        autoFocus
                      />
                      <button
                        onClick={() => handleRename(v.id)}
                        style={{ border: "none", background: "#2a78d6", color: "#fff", borderRadius: 6, padding: "3px 8px", fontSize: 11, cursor: "pointer" }}
                      >
                        保存
                      </button>
                      <button
                        onClick={() => setEditingId(null)}
                        style={{ border: "1px solid #d0d5dd", background: "#fff", color: "#667085", borderRadius: 6, padding: "3px 8px", fontSize: 11, cursor: "pointer" }}
                      >
                        取消
                      </button>
                    </div>
                ...
                <td style={{ padding: "8px", textAlign: "right", whiteSpace: "nowrap" }}>
                  <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                    ...
                    {canRenameDelete && editingId !== v.id && (
                      <>
                        <button
                          onClick={() => { setEditingId(v.id); setDraftName(v.name); }}
                          style={{ border: "1px solid #d0d5dd", background: "#fff", color: "#667085", borderRadius: 6, padding: "3px 10px", fontSize: 11.5, cursor: "pointer" }}
                        >
                          改名
                        </button>
                        <button
                          onClick={() => handleDelete(v)}
                          style={{ border: "1px solid #fecaca", background: "#fff", color: "#d03b3b", borderRadius: 6, padding: "3px 10px", fontSize: 11.5, cursor: "pointer" }}
                        >
                          删除
                        </button>
                      </>
