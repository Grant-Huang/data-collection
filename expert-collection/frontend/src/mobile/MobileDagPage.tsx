...
export function MobileDagPage({ active, onBack }: Props) {
  const [sheetNode, setSheetNode] = useState<GraphNode | null>(null);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#fff" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        <div style={{ borderBottom: "1px solid #e5e7eb", minHeight: 160 }}>
          {active ? (
            <DagView
              graph={active.graph}
              readOnly
              scrollable
              onNodeTap={setSheetNode}
              emptyLabel="还没有内容，滑回会话页开始讲述吧。"
            />
          ) : (
      ...
      {sheetNode && (
        <>
          <div onClick={() => setSheetNode(null)} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.35)", zIndex: 20 }} />
          <div style={{ position: "fixed", left: 0, right: 0, bottom: 0, background: "#fff", borderRadius: "16px 16px 0 0", padding: 20, zIndex: 21, boxShadow: "0 -4px 24px rgba(0,0,0,0.15)" }}>
            ...
            {sheetNode.actor_roles.length > 0 && (
              <div style={{ fontSize: 13, color: "#475569", marginBottom: 6 }}>涉及角色：{sheetNode.actor_roles.join("、")}</div>
            )}
