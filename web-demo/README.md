...
**2026-08-22 追加排查：不是干净的"需要间隔"规律，是真的不稳定。** 按上面的思路专门测了"间隔够不够"这个假设，同一个 check-in 周期里连续跑了 4 次独立连接：
1. 初始 update + 等 5 秒 + 第二次 update → ✅ 两次都成功拿到 `session.updated`。
2. 紧接着另一条新连接，初始 update 后**立即**（无间隔）发第一轮记忆 patch → ❌ 卡住超时，跟之前记录的一样。
3. 再测一次"初始 update + 显式等 3 秒再发第二次"，想确认"等一下就行"这个规律 → ❌ **这次连最初的第一次 `session.update`（连接后的第一条消息）都超时了**，还没轮到第二次。

三次结果互相矛盾：同样"等几秒再发"的做法，一次成功、一次还没测到第二步就先在第一步失败了。这说明**这不是一个"等够 N 秒就稳定"的干净规律**，而是服务本身当前处于时好时坏的状态——同样的操作序列，不同时刻的成功率不一样。比起"需要更长间隔"，更准确的描述是"目前这个服务不稳定，`session.update` 有一定概率（不管是第几次发送）会石沉大海，没有规律可循"。
