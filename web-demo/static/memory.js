...
// to localStorage.
//
// Every transcribed/typed user turn gets stored here, and later turns retrieve
// relevant past entries by keyword overlap + recency to ground the model's answer.
// Deliberately the simplest thing that validates the concept — no embeddings, no
// external sync, purely local to this browser.
...
const LocalMemory = (() => {
  ...
  return {
    /**
     * @param {string} text
     * @param {object} [meta] e.g. {isJargon: true, layer: "PROGRESS"}.
     * @returns the created entry, or undefined if the text was too short to store.
     */
    add(text, meta = {}) {
      const trimmed = (text || "").trim();
      if (trimmed.length < 2) return undefined;
      const entry = { id: crypto.randomUUID(), text: trimmed, timestamp: Date.now(), ...meta };
      entries.push(entry);
      persist(entries);
      return entry;
    },
